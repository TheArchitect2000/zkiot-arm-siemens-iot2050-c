...
echo "1. Generate Commitment and Build Executable"
echo "2. Generate ZKP"
echo "3. Verify ZKP"
...
case $user_choice in
    ...
    1)
        ...
        echo "[2/$total_steps] Finding the first instruction after 'main:' in program.s"
        # line_number=$(awk '/#APP/{flag=1; next} flag && /add|mul|sub|udiv|sdiv/{print NR; exit}' program.s)
        line_number=$(awk '/main:/{flag=1; next} flag && /add|mul|sub|udiv|sdiv/{print NR; exit}' program.s)
        if [ -z "$line_number" ]; then
            echo "No relevent instruction found after 'main:' in program.s"
            exit 1
        fi
        ...
        ./commitmentGenerator ./program.s > log/commitmentGenerator.log 2>&1
        if [ $? -ne 0 ]; then
            echo "commitmentGenerator execution failed"
            exit 1
        fi

        # Step 8: Build the program_AddedFidesProofGen.s using the updated codes and store the output logs
        echo "[8/$total_steps] Build the executable from program_AddedFidesProofGen.s"
        g++ -std=c++17 program_AddedFidesProofGen.s lib/polynomial.cpp -o program -lstdc++
        if [ $? -ne 0 ]; then
            echo "Build failed"
            exit 1
        fi
    ...
    3)
        ...
        echo "[3/$total_steps] Checking verification result"
