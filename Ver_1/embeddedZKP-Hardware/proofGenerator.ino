...
std::string vectorToJsonArray(const std::vector<uint64_t>& vec) {
  std::string result = "[";
...
void proofGenerator() {
  ...
  // }
  // char commitmentJsonFile[] = "\"Com_AHP0\":95085,\"Com_AHP1\":1458512,\"Com_AHP2\":95992,\"Com_AHP3\":1486087,\"Com_AHP4\":472594,\"Com_AHP5\":23750,\"Com_AHP6\":95085,\"Com_AHP7\":95085,\"Com_AHP8\":450947";
  // JsonDocument commitmentJsonData;
  // deserializeJson(commitmentJsonData, commitmentJsonFile);
  // Extract data from the parsed JSON
  uint64_t Class = 1;
  std::string commitmentID = "242dc27dc99524a33db5f4bf3997cd2f78c040033b32b7fc2552954097e272cf";
  std::vector<uint64_t> rowA_x = { 1539690, 252040, 1089890, 976702 };
  std::vector<uint64_t> colA_x = { 860159, 285701, 1418889, 592509 };
  std::vector<uint64_t> valA_x = { 1502728, 1258110, 918890, 1163508 };
  std::vector<uint64_t> rowB_x = { 502100, 1289630, 52300, 425431 };
  std::vector<uint64_t> colB_x = { 603791, 389452, 1162521, 696260 };
  std::vector<uint64_t> valB_x = { 1464026, 663717, 1005172, 1192583 };
  std::vector<uint64_t> rowC_x = { 1539690, 252040, 1089890, 976702 };
  std::vector<uint64_t> colC_x = { 1539690, 252040, 1089890, 976702 };
  std::vector<uint64_t> valC_x = { 1147974, 1381799, 1050419, 816594 };
  ...
  vector<uint64_t> nonZeroA = { 18, 0 };
  vector<vector<uint64_t>> nonZeroB = { { 33, 19, 1 }, { 34, 0, 28 }, { 34, 33, 1 } };
  vector<uint64_t> rowA = { 680600, 428200, 1400838, 471400 };
  vector<uint64_t> colA = { 1568397, 1, 1400838, 471400 };
  vector<uint64_t> valA = { 76653, 1167676, 0, 0 };
  vector<uint64_t> rowB = { 680600, 428200, 428200, 471400 };
  vector<uint64_t> colB = { 1263163, 1, 680600, 471400 };
  vector<uint64_t> valB = { 1147776, 917708, 612898, 0 };
  vector<uint64_t> rowC = { 680600, 428200, 1400838, 471400 };
  vector<uint64_t> colC = { 680600, 428200, 1400838, 471400 };
  vector<uint64_t> valC = { 1219064, 195110, 0, 0 };
  ...
  int64_t b = random(0, upper_limit);
  ...
  //     // std::cerr << "Error: " << e.what() << std::endl;
  // }
  vector<uint64_t> ck = { 17, 2023, 240737, 48205, 969812, 1009636, 982109, 884118, 345216, 1359179, 1267340, 1460526, 616745, 305049, 1345889, 1274691, 746434, 1438291, 1148502, 29692, 355626, 1009108, 919277, 1351415, 343424, 1145931, 1312604, 491498, 1289266, 891998, 1282936, 138728, 620022, 695012, 85656, 659898, 673673, 724037, 361909, 167924, 916624, 1035708, 906955, 1473958, 626292, 1441142, 1487771, 681178, 28271, 186527, 1541520, 721865, 103441, 1187452, 1487020, 591809, 515387, 954335, 756734, 1075130, 831590, 449828, 1097119, 270559, 419301, 642128, 147904, 123105, 349746, 309388, 273369 };
  uint64_t vk = 2023;


  // Measure the start time
  // auto start_time = high_resolution_clock::now();
  vector<uint64_t> z = { 1, 1, 1192032, 1192048, 1290179, 0, 376451, 0, 1191328, 538393, 1407617, 0, 0, 509963, 0, 1407489, 0, 0, 159, 25, 1343007, 0, 1289903, 0, 0, 0, 0, 0, 0, 0, 1191688, 628731, 0, 3975, 4003 };
  ...
  vector<vector<uint64_t>>().swap(A);
  vector<vector<uint64_t>>().swap(B);
  vector<vector<uint64_t>>().swap(C);
  ...
  vector<uint64_t>().swap(etaA_z_hatA_x);
  vector<uint64_t>().swap(etaB_z_hatB_x);
  vector<uint64_t>().swap(etaC_z_hatC_x);
  ...
  vector<uint64_t>().swap(Sum_M_eta_M_z_hat_M_x);
  ...
  vector<uint64_t>().swap(Sum_M_eta_M_r_M_alpha_x);
  vector<uint64_t>().swap(Sum_M_eta_M_r_M_alpha_x_z_hat_x);
  vector<uint64_t>().swap(Sum_check_protocol);
  ...
  doc["data"]["proof"]["commitmentId"] = commitmentID;
  doc["data"]["proof"]["class"] = Class;
  doc["data"]["proof"]["input"] = input_value;
  doc["data"]["proof"]["output"] = output_value;
  doc["data"]["proof"]["P_AHP1"] = sigma1;
  int counter = 0;
  for (auto i : w_hat_x) {
    doc["data"]["proof"]["P_AHP2"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : z_hatA) {
    doc["data"]["proof"]["P_AHP3"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : z_hatB) {
    doc["data"]["proof"]["P_AHP4"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : z_hatC) {
    doc["data"]["proof"]["P_AHP5"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : h_0_x) {
    doc["data"]["proof"]["P_AHP6"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : s_x) {
    doc["data"]["proof"]["P_AHP7"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : g_1_x) {
    doc["data"]["proof"]["P_AHP8"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : h_1_x) {
    doc["data"]["proof"]["P_AHP9"][counter] = i;
    counter++;
  }
  doc["data"]["proof"]["P_AHP10"] = sigma2;
  counter = 0;
  for (auto i : g_2_x) {
    doc["data"]["proof"]["P_AHP11"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : h_2_x) {
    doc["data"]["proof"]["P_AHP12"][counter] = i;
    counter++;
  }
  doc["data"]["proof"]["P_AHP13"] = sigma3;
  counter = 0;
  for (auto i : g_3_x) {
    doc["data"]["proof"]["P_AHP14"][counter] = i;
    counter++;
  }
  counter = 0;
  for (auto i : h_3_x) {
    doc["data"]["proof"]["P_AHP15"][counter] = i;
    counter++;
  }
  doc["data"]["proof"]["P_AHP16"] = y_prime;
  doc["data"]["proof"]["P_AHP17"] = p_17_AHP;
  counter = 0;
  for (auto i : Com1_AHP_x) {
    doc["data"]["proof"]["Com_AHP1_x"][counter] = i;
    counter++;
  }
  doc["data"]["proof"]["Com_AHP2_x"] = Com2_AHP_x;
  doc["data"]["proof"]["Com_AHP3_x"] = Com3_AHP_x;
  doc["data"]["proof"]["Com_AHP4_x"] = Com4_AHP_x;
  doc["data"]["proof"]["Com_AHP5_x"] = Com5_AHP_x;
  doc["data"]["proof"]["Com_AHP6_x"] = Com6_AHP_x;
  doc["data"]["proof"]["Com_AHP7_x"] = Com7_AHP_x;
  doc["data"]["proof"]["Com_AHP8_x"] = Com8_AHP_x;
  doc["data"]["proof"]["Com_AHP9_x"] = Com9_AHP_x;
  doc["data"]["proof"]["Com_AHP10_x"] = Com10_AHP_x;
  doc["data"]["proof"]["Com_AHP11_x"] = Com11_AHP_x;
  doc["data"]["proof"]["Com_AHP12_x"] = Com12_AHP_x;
  doc["data"]["proof"]["Com_AHP13_x"] = Com13_AHP_x;

  // serializeJson(proof, proofString);
