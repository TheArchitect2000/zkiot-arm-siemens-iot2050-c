...
uint64_t Polynomial::generateRandomNumber(const std::vector<uint64_t>& H, uint64_t p) {
    long rand = p - 1;
    rand %= p;
...
void NTT(vector<uint64_t>& a, bool invert, uint64_t p, uint64_t root) {
    int n = a.size();
    uint64_t root_inv = Polynomial::pExp(root, p - 2, p);  // Inverse of root p
    uint64_t root_pw = Polynomial::pExp(root, (p - 1) / n, p);  // root^(n/pw) p
